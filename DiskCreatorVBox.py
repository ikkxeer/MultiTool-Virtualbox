...
class VirtualBoxDiskCreator:
    ...
    def create_widgets(self):
        ...
        # Button to start the disk creation process
        self.create_button = tk.Button(self.root, text="Limpiar Discos", command=self.delete_disks, bg='#FF6961')
        self.create_button.pack(pady=(20, 0))

        # Button to start the disk creation process
        self.create_button = tk.Button(self.root, text="BORRAR MEDIOS", command=self.clean_media, bg='#FF6961')
        self.create_button.pack(pady=(20, 0))
    ...
    def clean_media(self):
        # Obtener una lista de UUIDs de discos asociados a máquinas virtuales
        vmUUIDs_raw = subprocess.check_output(["VBoxManage", "list", "vms"]).decode("utf-8")
        vmUUIDs = [line.split()[0].strip('"') for line in vmUUIDs_raw.splitlines()]

        # Obtener una lista de todos los medios registrados en VirtualBox
        hdds_raw = subprocess.check_output(["VBoxManage", "list", "hdds"]).decode("utf-8")
        hdds = [line.strip() for line in hdds_raw.splitlines()]

        # Filtrar y eliminar los medios no asociados a ninguna máquina virtual
        for line in hdds:
            if "UUID:" in line:
                uuid = line.split()[1].replace('"', '').replace(' ', '')  # Eliminar espacios en blanco
                if uuid not in vmUUIDs:
                    print(f"Eliminando disco con UUID: {uuid}")
    ...
    def delete_disks(self):
        ...
        for port_number in range(101):
            try:
                subprocess.run(["VBoxManage", "storageattach", selected_machine, "--storagectl", "SATA", "--port", str(port_number), "--medium", "none"], check=True, capture_output=True, text=True)
